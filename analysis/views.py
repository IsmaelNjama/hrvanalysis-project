...
def analysis_board(request, pk):
	...
	try:
		...
		return redirect('regenerate_result', pk=pk)
...
def regenerate_result(request, pk):
	"""
	Execute if result was not generated by save signals
	probably due to timeout
	"""
	sample = get_object_or_404(Sample, pk=pk)
	if request.method == 'POST':
		result = Result.objects.create(sample=sample)
		result.save()
		messages.info(request, f'Results were computed successfully')
		return redirect('analysis_board', pk=pk)
	return render(request, 'analysis/regen-result.html')
