...
def preview_file(request, pk):
	...
	if request.method == "POST":
		ecg_file = request.FILES.get('ecgFile')
		nni_file = request.FILES.get('nniFile')
		rpeaks_file = request.FILES.get('rpeaksFile')
		if ecg_file:
			try:
				filtered_signal, rpeaks = get_ecg_data(ecgfile=ecg_file)					
				# Compute NNI parameters
				nni = tools.nn_intervals(rpeaks)
				nni_data[ecg_file.name] = nni.tolist()
				# ECG plot
				fig = tools.plot_ecg(signal=filtered_signal, show=False)['ecg_plot']
				data_preview = fig_to_html(fig)
			except:
			...
			try:
				...
				nni_data[nni_file.name] = df.iloc[:, 0].dropna().tolist()
			except:
				messages.info(request, f'Something went wrong while trying to read the file(s)')
				return redirect('subject_sample_list', pk=subject.pk)
		elif rpeaks_file:
...
def result_render_pdf(request, pk):
	...
	para = f'attachment; filename="HRVSciHub_report_{now.strftime("%Y-%m-%d")}.pdf"'
